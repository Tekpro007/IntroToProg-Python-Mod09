...
# Kyle Gilpin, 6.8.2021, Imported function data from listing starter file
# Kyle Gilpin, 6.8.2021, Wrote function to items for a person
# ---------------------------------------------------------- #
...
class EmployeeIO:
    ...
    @staticmethod
    def print_current_list_items_person(list_of_rows):
        """ Print the current items in the list of Person rows

        :param list_of_rows: (list) of rows you want to display
        """
        print("******* The current items Person are: *******")
        for row in list_of_rows:
            print(str(row.first_name)
                  + ","
                  + row.last_name)
        print("*******************************************")
        print()  # Add an extra line for looks
